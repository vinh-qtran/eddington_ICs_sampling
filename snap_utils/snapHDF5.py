...
datablocks = { 	"POS ":["Coordinates",3], 
		...
		"BFLD":["MagneticField",3],
                #DUST
                "GDZ ":["GFM_DustMetallicity",1],
                "GDCM":["GFM_DustCapMass",1],
                "TASP":["GFM_DustTauSputter",1],
                "TAGR":["GFM_DustTauGrowth",1],
                "GLAM":["GFM_Lambda",6],
                "DGSD":["Dust_NumGrains",16],
                "DLHS":["Dust_DustHsml",1],
                "DDEN":["Dust_DustDensity",1],
                "DGDE":["Dust_GasDensity",1],
                #RT
		"PHOD":["PhotonDensity",1],
                "MACH":["Machnumber",1]
             }
...
class snapshot_header:
	def __init__(self, *args, **kwargs):
		if (len(args) == 1):
			...
			self.NTYPES =  6 #hdf5lib.GetAttr(f, "Config", "NTYPES")
			f.close()
		else:
			#read arguments
			...
			self.NTYPES = 6 #kwargs.get("NTYPES")

			#set default values
			if (self.npart.any() == None):
				self.npart = np.array([0,0,0,0,0,0], dtype="int32")
			if (self.nall.any() == None):				
			...
			if (hdf5lib.Contains(f,"",part_name)):
				...
				if ((block_name=="Masses") & (npart[parttype]>0) & (massarr[parttype]>0) & (no_mass_replicate==False)):
					...
					if (first):
						...
						dim1+=data.shape[0] #long(data.shape[0]) Python 2.7
						ret_val=data
						first=False
					else:	
						data=np.repeat(massarr[parttype], npart[parttype])
